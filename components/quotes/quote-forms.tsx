...
export function QuoteFinancialSettingsForm({ action, addresses, depositRateBasisPoints, discountRateBasisPoints, isQuoteFree, preparationFeeHtCents, preparationFeeVatRateBasisPoints, quoteId, travelFeeApplicable, validUntil, workAddressId }: { action: QuoteAction; addresses: { address_line_1: string; city: string; id: string; label: string | null; postal_code: string }[]; depositRateBasisPoints: number; discountRateBasisPoints: number; isQuoteFree: boolean | null; preparationFeeHtCents: number | null; preparationFeeVatRateBasisPoints: number | null; quoteId: string; travelFeeApplicable: boolean | null; validUntil: string | null; workAddressId: string | null }) {
  const [state, formAction] = useActionState(action, initialQuoteFormState);
  return (
