      ...
      <section className="rounded-2xl border border-border bg-background shadow-sm">
        ...
        <div className="space-y-5 p-5">
          ...
          {sections.map((section) => (
            <article className="space-y-4 rounded-xl border border-border bg-muted/10 p-4" key={section.id}>
              <QuoteSectionForm action={saveQuoteSection} quoteId={quoteId} section={section} />
              {(linesBySection.get(section.id) ?? []).map((line) => (
                <div className="space-y-3 rounded-xl border border-border bg-background p-4" key={line.id}>
                  <QuoteLineForm
                    action={saveQuoteLine}
          ...
          {unsectionedLines.length ? (
            <div className="space-y-3">
              <div>
                <h3 className="font-semibold">Lignes sans section</h3>
                <p className="text-sm text-muted-foreground">Prestations ajoutées directement au devis.</p>
              </div>
              {unsectionedLines.map((line) => (
                <article className="space-y-3 rounded-xl border border-border p-4" key={line.id}>
                  <QuoteLineForm
                    action={saveQuoteLine}
                    catalogItems={catalogItems}
                    line={line}
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-border bg-background shadow-sm">
      <div className="border-b border-border px-5 py-4">
        <h2 className="font-semibold">Contenu finalisé</h2>
        <p className="text-sm text-muted-foreground">Cette version est immuable.</p>
      </div>
      <div className="divide-y divide-border">
        {lines.map((line) => (
          <article className="p-5" key={line.id}>
  ...
  return (
    <section className="rounded-2xl border border-border bg-background p-5 shadow-sm">
      <div className="mb-4 flex items-center gap-2">
        <div className="rounded-xl bg-emerald-50 p-2 text-emerald-700"><ReceiptText className="size-4" /></div>
      ...
      {totals.isComplete ? (
        <div className="space-y-2.5 text-sm">
          ...
          {totals.vatBreakdown.map((vat) => (
            <div className="flex justify-between gap-4" key={vat.vatRateBasisPoints}>
              <span className="text-muted-foreground">TVA {vat.vatRateBasisPoints / 100} %</span>
              <span>{formatCents(vat.vatCents)}</span>
            </div>
          ))}
          <div className="flex justify-between gap-4 border-t border-border pt-3 text-base font-semibold text-emerald-700">
            <span>Total TTC</span>
            <span>{formatCents(totals.totalTtcCents)}</span>
          </div>
        ...
        <p className="text-sm text-muted-foreground">
          Total en attente : renseignez le prix HT et la TVA des lignes {totals.missingLineIndexes.map((index) => index + 1).join(", ")}.
        </p>
      )}
    </section>
  );
    ...
    <main className="min-h-svh bg-muted/20 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      <section className="mx-auto w-full max-w-[1500px] space-y-6">
        <div className="space-y-4">
          <Link className="inline-flex items-center gap-2 text-sm font-medium text-muted-foreground transition-colors hover:text-foreground" href="/devis">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-3">
                <h1 className="text-3xl font-semibold tracking-tight">
                  {editor.quote.quote_number ?? `Devis de ${customer?.display_name ?? "client"}`}
                </h1>
                <span className={`rounded-full px-3 py-1 text-xs font-semibold ${statusClassName}`}>{statusLabel}</span>
              </div>
              <p className="mt-2 text-sm text-muted-foreground">
                {finalized
                  ? "Devis finalisé et immuable."
                  : `Devis en cours de préparation pour ${customer?.display_name ?? "ce client"}.`}
              </p>
            </div>

            <div className="flex flex-col gap-2 sm:flex-row">
              {finalized && editor.quote.quote_version_id ? (
                <div className="rounded-lg border border-border bg-background px-1 py-1 shadow-sm">
                  <QuotePdfForm quoteId={editor.quote.id} versionId={editor.quote.quote_version_id} />
                </div>
              ) : null}
              {!finalized ? (
                <Link
                  className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-emerald-600 px-5 text-sm font-semibold text-white shadow-lg shadow-emerald-600/20 transition-all hover:bg-emerald-700"
                  href={`/devis/${editor.quote.id}/voix`}
                >
        ...
        <div className="grid gap-5 xl:grid-cols-[260px_minmax(0,1fr)_300px] xl:items-start">
          <aside className="space-y-5 xl:sticky xl:top-6">
            <section className="rounded-2xl border border-border bg-background p-5 shadow-sm">
              <div className="mb-4 flex items-center gap-2">
                <div className="rounded-xl bg-blue-50 p-2 text-blue-700"><UserRound className="size-4" /></div>
              ...
              <div className="mt-4 space-y-3 text-sm text-muted-foreground">
                ...
                {primaryAddress ? (
                  <p className="flex items-start gap-2">
                    ...
                    <span>{primaryAddress.address_line_1}{primaryAddress.address_line_2 ? `, ${primaryAddress.address_line_2}` : ""}<br />{primaryAddress.postal_code} {primaryAddress.city}</span>
                  </p>
                ) : null}
              </div>
            ...
            <section className="rounded-2xl border border-border bg-background p-5 shadow-sm">
              <div className="mb-4 flex items-center gap-2">
                <div className="rounded-xl bg-violet-50 p-2 text-violet-700"><FileText className="size-4" /></div>
                <h2 className="font-semibold">Informations du devis</h2>
              </div>
            ...
            {!finalized ? (
              <div className="rounded-2xl border border-amber-200 bg-amber-50/70 p-4 text-sm text-amber-900">
                ...
                <p className="mt-1">Utilisez la voix pour ajouter rapidement plusieurs prestations sans quitter le devis.</p>
              </div>
            ) : null}
          </aside>
          ...
          <section className="min-w-0 space-y-5">
            {!finalized ? (
              <section className="overflow-hidden rounded-2xl border border-border bg-background shadow-sm">
                <div className="border-b border-border px-5 py-4">
                  <h2 className="font-semibold">Paramètres du devis</h2>
                  <p className="text-sm text-muted-foreground">Conditions financières, adresse du chantier et notes visibles.</p>
                </div>
                <div className="p-5">
                  <QuoteFinancialSettingsForm
                ...
                <section id="acceptation">
                  {editor.quote.quote_version_id ? (
                    <QuoteAcceptancePanel
                      ...
                      action={recordQuoteAcceptance}
                      quoteId={editor.quote.id}
                      versionId={editor.quote.quote_version_id}
                    />
                  ) : null}
                </section>
              </>
            ...
            {!finalized ? (
              <section className="overflow-hidden rounded-2xl border border-border bg-background shadow-sm">
                <div className="border-b border-border px-5 py-4">
                  <h2 className="font-semibold">Assistant texte</h2>
                  <p className="text-sm text-muted-foreground">Vous pouvez également modifier le devis par conversation.</p>
                </div>
                <div className="p-5"><QuoteAssistant quoteId={editor.quote.id} /></div>
              </section>
            ) : null}
          </section>
          ...
          <aside className="space-y-5 xl:sticky xl:top-6">
            <section className="rounded-2xl border border-border bg-background p-5 shadow-sm">
              <div className="flex items-center justify-between gap-3">
                <h2 className="font-semibold">Statut du devis</h2>
                <span className={`rounded-full px-3 py-1 text-xs font-semibold ${statusClassName}`}>{statusLabel}</span>
              </div>
              <p className="mt-3 text-sm text-muted-foreground">
                {!finalized
                  ? "Ce devis peut encore être modifié avant sa finalisation."
                  : acceptance
                    ? "L’acceptation commerciale de ce devis a été enregistrée."
                    : "Le devis est finalisé et attend éventuellement l’acceptation du client."}
              </p>
            ...
            {!finalized && compliance ? (
              <section className="rounded-2xl border border-border bg-background p-5 shadow-sm">
                <h2 className="font-semibold">Finalisation</h2>
                <p className="mb-4 mt-1 text-sm text-muted-foreground">Vérifiez la conformité puis attribuez le numéro définitif du devis.</p>
                <FinalizeQuoteForm action={finalizeQuote} compliance={compliance} quoteId={editor.quote.id} />
              </section>
            ) : null}
            ...
            {!finalized ? (
              <section className="rounded-2xl border border-border bg-background p-5 shadow-sm">
                ...
                <p className="mb-3 mt-1 text-sm text-muted-foreground">La suppression est définitive et réservée aux brouillons.</p>
                <DeleteDraftQuoteForm
                  action={deleteDraftQuote}
                  customerName={customer?.display_name ?? "ce client"}
                  quoteId={editor.quote.id}
                />
              </section>
            ) : null}
            ...
            {finalized && acceptance ? (
              <section className="rounded-2xl border border-emerald-200 bg-emerald-50/60 p-5">
                <div className="flex items-center gap-2 text-emerald-800"><CheckCircle2 className="size-5" /><p className="font-semibold">Devis accepté</p></div>
                <p className="mt-2 text-sm text-emerald-900/80">L’accord du client est enregistré et conservé dans l’historique du devis.</p>
              </section>
            ) : null}
          </aside>
        </div>
