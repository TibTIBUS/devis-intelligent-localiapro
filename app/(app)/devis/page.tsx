    ...
    <main className="flex min-h-svh justify-center px-6 py-12">
      <section className="w-full max-w-3xl space-y-8">
        <div className="space-y-3">
          ...
          <h1 className="text-3xl font-semibold tracking-tight">Devis enregistrés</h1>
          <p className="text-sm text-muted-foreground">Retrouvez et reprenez les devis de votre entreprise.</p>
          <Link className="inline-block text-sm font-medium underline" href="/devis/nouveau">Créer un devis</Link>
        </div>
        <form action="/devis" className="flex flex-wrap gap-3">
          <label className="sr-only" htmlFor="quote-search">Rechercher un client</label>
          <input className="h-10 min-w-52 flex-1 rounded-md border border-input bg-background px-3 text-sm" defaultValue={search} id="quote-search" maxLength={100} name="recherche" placeholder="Rechercher par client" type="search" />
          <button className="h-10 rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground" type="submit">Rechercher</button>
        </form>
        {quotes.length ? (
          ...
          <p className="rounded-lg border border-border p-5 text-sm text-muted-foreground">{search ? "Aucun devis ne correspond à cette recherche." : "Aucun devis enregistré pour le moment."}</p>
        )}
      </section>
    </main>
  );
