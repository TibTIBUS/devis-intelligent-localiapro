...
export default async function CustomersPage({ searchParams }: { searchParams: Promise<{ client?: string; nouveau?: string; q?: string }> }) {
  ...
  return (
    <main className="min-h-svh bg-muted/20 px-3 py-5 min-[375px]:px-4 sm:px-6 lg:px-8">
      <section className="mx-auto w-full max-w-[1500px] space-y-5 sm:space-y-6">
        <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <Link className="inline-flex min-h-11 items-center justify-center rounded-xl bg-primary px-4 text-sm font-medium text-primary-foreground transition hover:bg-primary/80" href="/clients?nouveau=1">+ Nouveau client</Link>
        </header>

        <section className="grid gap-3 min-[420px]:grid-cols-3">
          <article className="rounded-xl border border-border bg-background p-4 shadow-sm"><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Clients</p><p className="mt-2 text-2xl font-semibold">{customers.length}</p><p className="mt-1 text-xs text-muted-foreground">Enregistrés</p></article>
          <article className="rounded-xl border border-border bg-background p-4 shadow-sm"><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Contacts</p><p className="mt-2 text-2xl font-semibold">{totalContacts}</p><p className="mt-1 text-xs text-muted-foreground">Coordonnées enregistrées</p></article>
          <article className="rounded-xl border border-border bg-background p-4 shadow-sm"><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Adresses</p><p className="mt-2 text-2xl font-semibold">{totalAddresses}</p><p className="mt-1 text-xs text-muted-foreground">Chantiers et adresses client</p></article>
        </section>
        ...
        <section className="grid gap-5 lg:grid-cols-[280px_minmax(0,1fr)] xl:grid-cols-[320px_minmax(0,1fr)_320px] xl:items-start">
          <aside className="rounded-xl border border-border bg-background p-4 shadow-sm lg:sticky lg:top-24 lg:max-h-[calc(100svh-7rem)] lg:overflow-y-auto">
            ...
            <div className="space-y-2">
              {filteredCustomers.length ? filteredCustomers.map((customer) => {
                ...
                return (
                  <Link className={`block rounded-lg border p-3 transition ${isSelected ? "border-primary/40 bg-primary/5 shadow-sm" : "border-transparent hover:border-border hover:bg-muted/40"}`} href={`/clients?client=${customer.id}`} key={customer.id}>
                    <div className="flex items-start gap-3">
                      <div className="flex size-10 shrink-0 items-center justify-center rounded-full bg-muted text-sm font-semibold text-foreground">{initials(customer.display_name) || "CL"}</div>
          ...
          <section className="min-w-0 rounded-xl border border-border bg-background shadow-sm">
            <div className="border-b border-border px-4 py-4 sm:px-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div><h2 className="text-lg font-semibold">{params.nouveau === "1" ? "Nouveau client" : "Informations client"}</h2><p className="mt-1 text-xs text-muted-foreground">{params.nouveau === "1" ? "Créez la fiche, puis ajoutez ses coordonnées." : "Modifiez la fiche et les coordonnées enregistrées."}</p></div>
                {selectedCustomer && params.nouveau !== "1" ? <div className="flex size-12 items-center justify-center rounded-full bg-muted text-base font-semibold">{initials(selectedCustomer.display_name) || "CL"}</div> : null}
              </div>
            </div>
          ...
          <aside className="grid gap-5 lg:col-span-2 lg:grid-cols-2 xl:sticky xl:top-24 xl:col-span-1 xl:block xl:space-y-5">
            ...
            <section className="rounded-xl border border-border bg-background p-4 shadow-sm">
              <div className="flex items-center justify-between gap-3"><h2 className="text-base font-semibold">Derniers devis</h2>{selectedCustomer ? <Link className="text-xs font-medium text-primary hover:underline" href="/devis">Voir tous</Link> : null}</div>
              <div className="mt-4 space-y-2">
                {quotes?.length ? quotes.map((quote) => <Link className="block rounded-lg border border-border p-3 transition hover:bg-muted/40" href={`/devis/${quote.id}`} key={quote.id}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-semibold">{quote.quote_number ?? "Devis brouillon"}</p><p className="mt-1 text-xs text-muted-foreground">{formatDate(quote.issued_on)}</p></div><span className={`rounded-full px-2 py-1 text-[11px] font-medium ${quote.status === "finalized" ? "bg-emerald-50 text-emerald-700" : "bg-muted text-muted-foreground"}`}>{quote.status === "finalized" ? "Finalisé" : "Brouillon"}</span></div></Link>) : <p className="rounded-lg border border-dashed border-border p-4 text-sm text-muted-foreground">{selectedCustomer ? "Aucun devis pour ce client." : "Sélectionnez un client."}</p>}
              </div>
            </section>
          </aside>
        </section>
